import { Settings, Info, Github, RefreshCw } from 'lucide-react'
        ...
        <div className="p-6 space-y-6 overflow-y-auto h-full">
                ...
                <div className="space-y-4">
                    <label className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-xs text-zinc-500">Automatically connect to the last used device</p>
                        </div>
                        <input
                            type="checkbox"
                            checked={settings.autoConnectLastDevice}
                            onChange={(e) => settings.updateSettings({ autoConnectLastDevice: e.target.checked })}
                    ...
                    <label className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-xs text-zinc-500">Display system apps in the app list by default</p>
                        </div>
                        <input
                            type="checkbox"
                            checked={settings.showSystemApps}
                            onChange={(e) => settings.updateSettings({ showSystemApps: e.target.checked })}
                            className="w-5 h-5 rounded border-border bg-bg-tertiary text-accent"
                        />
                    </label>
                </div>
            ...
            <div className="bg-bg-secondary border border-border rounded-xl p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label className="block text-sm text-zinc-400 mb-2">Default Resolution</label>
                        <select
                            value={settings.defaultScrcpyResolution}
                            onChange={(e) => settings.updateSettings({ defaultScrcpyResolution: parseInt(e.target.value) })}
                    ...
                    <div>
                        <label className="block text-sm text-zinc-400 mb-2">Default Bitrate (Mbps)</label>
                        <Input
                            type="number"
                            value={settings.defaultScrcpyBitrate}
                            onChange={(e) => settings.updateSettings({ defaultScrcpyBitrate: parseInt(e.target.value) })}
                            min={1}
                            max={100}
                        />
                    </div>
            ...
            <div className="bg-bg-secondary border border-border rounded-xl p-6">
                    ...
                    {[
                        ...
                        <div key={name} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                            ...
                            <div className="flex items-center gap-2">
                                {info?.isInstalled ? (
                                    <>
                                        <span className="text-xs text-zinc-500">v{info.version}</span>
                ...
                <Button variant="secondary" className="mt-4" onClick={handleCheckUpdates}>
