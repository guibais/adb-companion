...
import { Download, CheckCircle, AlertCircle, Loader2, ChevronRight, RefreshCw, ExternalLink } from 'lucide-react'
...
function BinaryItem({ displayName, info, progress, required, onRetry, isRetrying }: BinaryItemProps) {
    const isDownloading = progress?.status === 'downloading' || progress?.status === 'extracting'
        ...
        <div className="bg-bg-tertiary rounded-lg p-4">
            <div className="flex items-center justify-between mb-2">
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            {required && !isCompleted && (
                                <span className="text-xs bg-yellow-500/20 text-yellow-400 px-1.5 py-0.5 rounded">Required</span>
                            )}
                        </div>
                        <p className="text-xs text-zinc-500">v{info?.version || 'unknown'}</p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <span className={`text-xs px-2 py-1 rounded-full ${isCompleted ? 'bg-green-500/20 text-green-400' :
                        isFailed ? 'bg-red-500/20 text-red-400' :
                            isDownloading || isRetrying ? 'bg-blue-500/20 text-blue-400' :
                                'bg-zinc-500/20 text-zinc-400'
                        }`}>
                        {isCompleted ? 'Installed' :
                            isFailed ? 'Failed' :
                                isRetrying ? 'Retrying...' :
                                    isDownloading ? (progress?.status === 'extracting' ? 'Extracting...' : 'Downloading...') :
                                        'Pending'}
                    </span>
                </div>
            </div>
            ...
            {isFailed && progress?.error && (
                <p className="text-xs text-red-400 mt-2 bg-red-500/10 p-2 rounded">{progress.error}</p>
            )}
        </div>
    )
}
...
export function SetupScreen() {
    const { status, setStatus, isDownloading, setDownloading } = useBinaryStore()
    ...
    useEffect(() => {
        ...
        const unsubscribe = window.electronEvents?.['download:progress']?.((progress: DownloadProgress) => {
            setProgressMap(prev => {
    ...
    const checkBinaries = async () => {
        try {
            ...
            setError('Failed to check installed tools. Please restart the application.')
        }
    }
    ...
    const handleDownloadAll = async () => {
        ...
        try {
            ...
            setError(`Download failed: ${err instanceof Error ? err.message : 'Unknown error'}. You can retry individual tools below.`)
        } finally {
    ...
    const allRequired = status?.platformTools.isInstalled && status?.scrcpy.isInstalled
    const allOptional = status?.java.isInstalled && status?.apktool.isInstalled && status?.uberApkSigner.isInstalled
    ...
    const hasAnyFailed = Array.from(progressMap.values()).some(p => p.status === 'failed')
        ...
        <div className="min-h-screen max-h-screen bg-bg-primary overflow-y-auto">
            <div className="w-full max-w-lg mx-auto p-8 py-12">
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-2xl font-bold text-white mb-2">Welcome to ADB Companion</h1>
                    <p className="text-zinc-400">
                        We need to download some tools to get started. This only happens once.
                    </p>
                </div>
                ...
                <div className="mb-4 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-white font-medium">{installedCount}/5</span>
                    </div>
                </div>
                ...
                <div className="mt-6 p-4 bg-bg-secondary rounded-lg border border-border">
                    <div className="flex items-start gap-2 mb-2">
                        ...
                        <p className="text-xs text-zinc-400">
                            <strong className="text-zinc-300">Required:</strong> Platform Tools & scrcpy are needed for basic device communication and screen mirroring.
                        </p>
                    </div>
                    <div className="flex items-start gap-2">
                        <div className="w-2 h-2 rounded-full bg-zinc-500 mt-1.5" />
                        <p className="text-xs text-zinc-500">
                            <strong className="text-zinc-400">Optional:</strong> Java, APK Tool & Signer enable APK cloning/modification features.
                        </p>
                    </div>
                </div>
                ...
                <div className="mt-4 flex items-center justify-center gap-4 text-xs text-zinc-600">
                    ...
                    <span>•</span>
                    <button
                        onClick={() => window.electronAPI['shell:open-external']('https://github.com/adb-companion/adb-companion')}
